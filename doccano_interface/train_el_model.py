...
class BERTNERComponent(object):
    ...
    name = "bert_ner_component"  # component name, will show up in the pipeline

    def __init__(self, nlp, bert_model=None):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(bert_model, do_lower_case=False, use_fast=True)
    ...
    def __call__(self, doc: Doc):
        ...
        """
        matches = self.find_spans(doc.text)
        spans = []  # keep the spans for later so we can merge them afterwards
        for start, end, label in matches:
            # Generate Span representing the entity & set label
            entity = doc.char_span(start, end, label=self.label_map[label])
            spans.append(entity)
    ...
    def find_spans(self, text: str):
        ...
        sentence_tokens_for_model = self.tokenizer(text,
        ...
        sentence_tokens_for_postprocessing = self.tokenizer(text,
                                                            add_special_tokens=True,
                                                            padding='max_length',
                                                            truncation=True,
                                                            max_length=128,
        ...
        result: Tuple[Tensor] = self.model(**sentence_tokens_for_model)
...
def main(kb_path, vocab_path, output_dir=None, n_iter=50):
    ...
    ner_component = BERTNERComponent(nlp, "./model")
    nlp.add_pipe(ner_component)
    print("Added BERT NER pipe")
